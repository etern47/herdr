...
`pane.clear` clears the target pane's screen and scrollback like a native
terminal Cmd+K. The line the cursor is on (for example a shell prompt with
pending input) is kept and moved to the top of the pane. Nothing is written to
the pane's process. Panes on the alternate screen (full-screen apps) are left
untouched:

```json
{"id":"req_clear","method":"pane.clear","params":{"pane_id":"w1:p1"}}
```

The response is `type: "pane_clear"` with `cleared` set to false when the pane
was on the alternate screen and no clear happened.
