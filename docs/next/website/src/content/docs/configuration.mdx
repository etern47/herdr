...
`clear_pane` clears the focused pane like a native terminal Cmd+K: the visible output and the scrollback are erased, and the line you are typing on moves to the top of the pane. Nothing is sent to the shell or the running process, so it works with any prompt and any pending input; full-screen apps on the alternate screen are left untouched. It is unset by default; bind it to a free chord such as `clear_pane = "prefix+shift+c"` (`prefix+shift+k` is already `swap_pane_up`).
